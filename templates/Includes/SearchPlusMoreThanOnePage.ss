...
<p class="searchPlusMoreThanOnePageNumbers">
	<span class="searchPlusMoreThanOnePageSummary">Page $Results.CurrentPage of $Results.TotalPages</span>
 	<span class="prevNextNumbers">
		...
		<span class="numbers"><% loop Results.Pages %><% if CurrentBool %><span class="currentNumber">$PageNum</span><% else %><a href="$Link" title="View page number $PageNum" class="notCurrentNumber">$PageNum</a><% end_if %><% end_loop %></span>
